...
# QPINN wrapper (your full qnode-based model)
# Here we assume `QPINN` is exactly your class defined above.
# If you want to use the real quantum circuit, just import it instead.
class QPINN(nn.Module):
    def __init__(self):
        super().__init__()
        # copy your phase-parameter definitions here
...
# Instantiate
pinn_model  = PINN([2,50,50,50,50,50,50,50,1])
qpinn_model = QPINN()
epochs = 1000
cos_epochs = 150
...
for epoch in range(epochs):
    ...
    lbc2_p   = mse(pinn_model(tb2, xb2),
                   torch.exp(-k * (T - tb2)) / gamma)
    loss_p   = lpde_p + lbc1_p + 5 * lbc2_p
    opt_pinn.zero_grad(); loss_p.backward()
    ...
    lbc2_q   = mse(qpinn_model(tb2, xb2),
                   torch.exp(-k * (T - tb2)) / gamma)
    loss_q   = lpde_q + lbc1_q + 5 * lbc2_q
    opt_qpinn.zero_grad(); loss_q.backward()
